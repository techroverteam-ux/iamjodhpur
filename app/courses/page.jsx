  ...
  const loadCourses = async () => {
    try {
      ...
      try {
        ...
        if (bannerData.banners?.courses) {
          setBannerImage(bannerData.banners.courses)
        }
      } catch (error) {
    ...
    <>
      ...
      {bannerImage ? (
        <div style={{position: 'relative', width: '100%', overflow: 'hidden'}}>
          <img src={bannerImage} alt="Courses" style={{width: '100%', height: '400px', objectFit: 'cover', display: 'block'}} />
          <div style={{position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(255, 255, 255, 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
            <h1 style={{color: '#1B5A96', fontSize: '2.8rem', fontWeight: '900', textShadow: '2px 2px 4px rgba(0,0,0,0.3)'}}>Our Courses</h1>
          </div>
        </div>
      ) : (
